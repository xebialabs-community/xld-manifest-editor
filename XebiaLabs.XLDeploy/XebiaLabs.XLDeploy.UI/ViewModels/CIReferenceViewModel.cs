...
{
    public class CIReferenceViewModel : PropertyEntryEditorViewModel
    {
        public string ReferencedCI { get; set; }
        ...
        {
            if (PropertyDescriptor.Required && string.IsNullOrWhiteSpace(ReferencedCI))
            {
                return RequiredMsg;
            }

            if (string.IsNullOrWhiteSpace(ReferencedCI))
            {
                RemoveEntryProperty();
                return new string[0];
            }

            GetEntryProperty(true).SetCIReferenceValue(ReferencedCI);
            return new string[0];
        }
    }
}
