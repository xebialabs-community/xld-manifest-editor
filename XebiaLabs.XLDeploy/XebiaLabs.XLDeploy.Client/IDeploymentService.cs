...
namespace XebiaLabs.Deployit.Client
{
    public interface IDeploymentService
    {
        ...
        Deployment PrepareUpdate(string versionId, string deploymentId);
        Deployment AutoPrepareDeployeds(Deployment deployment);
        Deployment Validate(Deployment deployment);
        string GenerateDeploymentTask(Deployment deployment);
        string GenerateRollbackTask(string deploymentTaskId);
